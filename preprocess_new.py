...
def prepare_final_dataset(df, keep_city=False):
    ...
    df = df.drop(columns=na_rate[na_rate > 0.3].index)
    logging.info(f"shape after dropping columns with many missing values:\n{df.shape}")
    df = df.dropna(how='any')
    logging.info(f"shape after dropping rows with missing values:\n{df.shape}")
